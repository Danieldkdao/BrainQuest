...
import userModel, { createNewWeekPuzzles, createNewWeekPoints, createNewWeekTimeSpent, calcDaysTillSun, } from "../models/user.model.js";
import badgeModel from "../models/badge.model.js";
import { Levels } from "./user.controller.js";
import { PointsReference, BadgeReference, ChallengeReference, } from "../utils/reference.js";
import challengeModel from "../models/challenge.model.js";
export const addNewWeeks = async (userId, numDate, correctPuzzles, incorrectPuzzles, pointsEarned, timeSpent) => {
    try {
...
const updatePercentageText = async (userId) => {
    try {
        const categoryDataObject = await userModel.findOne({ userId }, { puzzleCategoryData: 1, _id: 0 });
        ...
        await userModel.updateOne({ userId }, { $set: { puzzleCategoryData: newData } });
    }
    catch (error) {
        console.error("Update percentage text error: ", error);
    }
};
const checkEarnedBadges = async (userId) => {
    try {
        const badges = await badgeModel.find();
        if (!badges || badges.length === 0)
            return;
        badges.forEach(async (item) => {
            await BadgeReference[item.condition](userId, item._id);
        });
    }
    catch (error) {
...
const updateLevel = async (userId, level, nextPoints, nextPuzzles) => {
    try {
        let query = {
            userId,
...
const checkUpdateLevel = async (userId) => {
...
const updateUser = async (userId, correctPuzzles, incorrectPuzzles, pointsEarned, timeSpent, categories) => {
    ...
    try {
        ...
        if (weeks[0].to < numDate) {
            await addNewWeeks(userId, numDate, correctPuzzles, incorrectPuzzles, pointsEarned, timeSpent);
        }
        const operations = categories.map((item) => {
            ...
            return {
                updateOne: {
                    filter: {
                        userId,
                        "puzzleCategoryData.label": item.category.charAt(0).toUpperCase() + item.category.slice(1),
                    },
                    update,
                },
            };
...
export const checkAnswer = async (req, res) => {
    ...
    const { puzzle, response, answer, difficulty, category, id, timeTaken, isDaily, } = req.body;
    try {
        if (id && userId) {
        ...
        const checkResponse = await axios.post("https://openrouter.ai/api/v1/chat/completions", {
            ...
            messages: [
                {
                    role: "system",
                    content: "You leniently compare the meaning of a response to a puzzle and it's answer and only return the strings true or false.",
                },
                {
                    role: "user",
        ...
        if (id && Number.isFinite(timeTaken) && typeof timeTaken !== "undefined") {
            ...
            await updateUser(userId, correctPuzzles, incorrectPuzzles, pointsEarned, timeTaken, [
                {
                    category,
        ...
        res.json({
            success: false,
            message: "Something went wrong while your answer was being checked. Please try again later.",
        });
    }
};
export const saveTrainingSession = async (req, res) => {
    try {
        const { userId } = getAuth(req);
        const { pointsEarned, puzzlesAttempted, puzzlesSolved, timeLimit, timeTaken, timeTakenNumber, allPuzzlesAnswered, } = req.body;
        ...
        await Promise.all([
            newSession.save(),
            updateUser(userId, puzzlesSolved, puzzlesAttempted - puzzlesSolved, pointsEarned, timeTakenNumber, allPuzzlesAnswered),
        ]);
        res.json({
            success: true,
