...
export const enableLeaderboard = async (req, res) => {
    try {
        const { userId } = getAuth(req);
        await userModel.updateOne({ userId }, [
...
export const updatePuzzleGoal = async (req, res) => {
    try {
        const { userId } = getAuth(req);
...
export const updatePointsGoal = async (req, res) => {
    try {
        const { userId } = getAuth(req);
        const { newValue } = req.body;
...
export const fetchUsers = async (req, res) => {
    try {
        const users = await userModel
            .find({ enableLeaderboard: true }, { userId: 1, name: 1, points: 1, puzzles: 1, _id: 0 })
...
export const checkResetStreak = async (req, res) => {
    try {
        const { userId } = getAuth(req);
        const now = Date.now();
        const user = await userModel.findOne({ userId }, {
        ...
        const puzzleWeeks = await userModel.findOne({ userId }, { "weekPoints.to": 1, _id: 0 });
